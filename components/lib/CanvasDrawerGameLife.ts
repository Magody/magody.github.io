...
export class CanvasDrawerGameLife extends CanvasDrawer {
  ...
  tryToDrawCell(x: number, y: number) {
    ...
    const columna_pulsada = Math.floor((x - offsetLeft) / this.lado_celula);

    const fila_pulsada = Math.floor((y - offsetTop) / this.lado_celula);

    const wasValidClick =
      columna_pulsada > 0 &&
      columna_pulsada < this.columnas - 1 &&
      fila_pulsada > 0 &&
      fila_pulsada < this.filas - 1;
